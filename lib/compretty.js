...
export class CompRettyTeams {
  ...
  logEvent(channelID, event) {
    ...
    if (_event != null) if (event.team == _event.team) return;
    _events[event.playerID] = event;
    burstStack10.set(_stackName, (_channelID = channelID) => {
      return this._logEvents(_channelID, _events);
    });
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'join':
      ...
      if (this.#onSave != null) this.#onSave();
      this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> joined team ${_teamName}.`));
      return await this.teamsWizard(interaction, isAdmin, true, _teamName);
    case 'join_random':
      var _availableTeams = [];
    ...
    case 'leave':
      ...
      if (_team != null) this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> left team ${_teamName}.`));
      if (_arg3 == 'minimalist') {
