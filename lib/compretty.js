...
const commands = [
  ...
  {
    name: 'Remove competition readme',
    dm_permission: false,
    type: 3,
    default_member_permissions: '0',
    default_permission: false,
...
const getHelpMsg = (isAdmin = false) => {
  ...
  return (`{"type":4,"data":{"flags":${1 << 6},"embeds":[{` +
    ...
    `\`/ready\` - ${readyDesc}\\n` +
    `\`/unready\` - ${unreadyDesc}\\n` +
    `\`/get_role\`, \`/role\`, \`/pug_role\` - ${getRoleDesc}\\n` +
    `\`/shuffle\` - ${shuffleDesc}\\n` +
...
export class CompRettyTeams {
  ...
  logEvent(guildID, channelID, event) {
    const _nickStackName = `nick${guildID}`;
    burstStack10.set(_nickStackName, () => {
      const guild = db.get(Path.join('usrprefs', 'guilds', guildID, 'guild.json'));
      var botNickname = guild.botNickname;
      if (botNickname == null) botNickname = 'CompRetty';
      if (this.playerCount == 0) this.#rest.modifyCurrentMember(guildID, `{"nick":"${botNickname}"}`);
      else this.#rest.modifyCurrentMember(guildID, `{"nick":"${botNickname} (${this.playerCount} ${this.playerCount == 1 ? 'player' : 'players'} entered)"}`);
    });
    ...
    const _stackName = `log${channelID}`;
  ...
  async onPlayerLeave(interaction) {
    var _playerID = interaction.member.user.id;
    var _username = getNickname(interaction.member);
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'ping':
      ...
      switch (_arg3) {
      case 'not_ready':
        const pingNotReadyTime = this.#pingNotReadyTime[_teamName];
        if (pingNotReadyTime != null) {
          // 5 minute cooldown
          if ((Date.now() - pingNotReadyTime) < 1000 * 60 * 5) {
            return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`${_teamName == 'cr_queue' ? 'Player queue' : `Team ${_teamName}`} has been pinged recently, cooldown in effect. :no_entry_sign:`));
          }
        }
        this.#pingNotReadyTime[_teamName] = Date.now();
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, queue = null, queueOnly = false, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null, moderatorRoleCheck = true, readme = null) {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, queue = null, queueOnly = false, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null, moderatorRoleCheck = true, readme = null) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, queue, queueOnly, shuffle, competitionMessageID, logEnabled, startOnShuffle, autoClearThreshold, autoClearDate, moderatorRoleCheck, readme);
  ...
  static fromJSON(json, rest = null) {
    ...
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _queue, json.queueOnly, _shuffle, json.competitionMessageID, json.logEnabled, json.startOnShuffle, json.autoClearThreshold, _autoClearDate, json.moderatorRoleCheck, json.readme);
    return _competition;
  }
  ...
  async checkQueueOnly(interaction) {
    ...
    if (_customID == null) {
      ...
      case 'Kick mentioned player':
        result = false;
        break;
      case 'Set competition readme': 
        result = false;
        break;
      case 'Remove competition readme':
        result = false;
        break;
      }
    }
  ...
  async execute(interaction, isAdmin = false) {
    ...
    if (this.#autoClearDate != null) {
      if (this.#teams.playerCount != 0) {
        ...
        if (_now.getTime() > this.#autoClearDate.getTime()) {
          ...
          const guild = db.get(Path.join('usrprefs', 'guilds', interaction.guild_id, 'guild.json'));
          var botNickname = guild.botNickname;
          if (botNickname == null) botNickname = 'CompRetty';
          this.#rest.modifyCurrentMember(interaction.guild_id, `{"nick":"${botNickname}"}`);
    ...
    case 'Set competition readme':
      if (!isAdmin) return await this.sendPermissionDenied(interaction);
      if (interaction.data == null) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      if (interaction.data.resolved == null) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      if (interaction.data.resolved.messages == null) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      if (interaction.data.resolved.messages.length == 0) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      var message = Object.values(interaction.data.resolved.messages)[0];
      if (message == null) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      if (message.content == null) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      if (message.content.length == 0) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Unable to set competition readme. :exploding_head:\nOnly simple messages allowed.`));
      this.#readme = message.content.substring(0, 2000);
      this.saveSync();
      return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Competition readme set. :notepad_spiral:`));
    case 'Remove competition readme': 
      if (!isAdmin) return await this.sendPermissionDenied(interaction);
      this.#readme = null;
      this.saveSync();
      return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Competition readme removed. :notepad_spiral:`));
    }
    var _customID = interaction.data.custom_id;
    ...
    if (_arg1 == 'teams') {
      return await this.#teams.execute(interaction, isAdmin || !this.#moderatorRoleCheck);
    } else if (_arg1 == 'readme') {
      if (this.#readme == null) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse('No readme set for this competition.'));
      else return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(this.#readme));
    }
  }
...
export class CompRettyChannel {  
  ...
  execute(interaction, isAdmin = false) {
    ...
    case 'role': return this._competitionExecute(interaction, isAdmin);
    case 'pug_role': return this._competitionExecute(interaction, isAdmin);
    ...
    case 'Set competition readme': return this._competitionExecute(interaction, isAdmin);
    case 'Remove competition readme': return this._competitionExecute(interaction, isAdmin);
    }
...
export class CompRettyGuild {
  ...
  get botNickname() { return this.#botNickname; }

  constructor(id, adminRole = null, channels = {}, botNickname = 'CompRetty') {
    this.#id = id;    
  ...
  static withREST(rest, id, adminRole = null, channels = {}, botNickname = 'CompRetty') {
    var _guild = new CompRettyGuild(id, adminRole, channels, botNickname);
    _guild.#rest = rest;
    return _guild;
  }
  ...
  static fromJSON(json, rest = null) {
    ...
    var _botNickname = json.botNickname;
    if (_botNickname == null) _botNickname = 'CompRetty';
    _botNickname = _botNickname.substring(0, 10);
    return CompRettyGuild.withREST(rest, json.id, json.adminRole, _channels, _botNickname);
  }
  ...
  */
  ...
  execute(interaction) {
    ...
    case 'role': return this._channelExecute(interaction, _isAdmin);
    case 'pug_role': return this._channelExecute(interaction, _isAdmin);
    ...
    case 'Set competition readme': return this._channelExecute(interaction, _isAdmin);
    case 'Remove competition readme': return this._channelExecute(interaction, _isAdmin);
    }
  }
        ...
        components: [
          {
            ...
            components: [
              {
                type: 2,                
                custom_id: 'setup guild bot_nickname',
                style: 2,
                label: 'Bot nickname',
                emoji: {
                  id: null,
  ...
  _botNicknameModal(interaction) {
    return this.#rest.createInteractionResponse(interaction, getInteractionResponse(
      getSimpleModal(
        'Bot nickname',
        'setup guild bot_nickname',
        {
          type: 4,
          custom_id: 'nickname',
          label: 'Bot nickname',
  ...
  setup(interaction) {
    ...
    if (_arg1 == 'guild') {
      ...
      case 'bot_nickname':
        if (!isAdmin(interaction.member)) return this.sendPermissionDenied(interaction);
        const _components = interaction.data.components;
        if (_components == null) return this._botNicknameModal(interaction);
        this.#botNickname = _components[0].components[0].value;
        if (this.#botNickname != null) this.#botNickname = this.#botNickname.substring(0, 10);
        this.saveSync();
        const _nickStackName = `nick${this.#id}`;
        burstStack10.set(_nickStackName, () => {
          var botNickname = this.#botNickname;
          if (botNickname == null) botNickname = 'CompRetty';
          this.#rest.modifyCurrentMember(this.#id, `{"nick":"${botNickname}"}`);
        });
        return this.setupWizard(interaction, true);
      }
...
export class CompRetty {
  ...
  executeInteraction(interaction) {
    ...
    case 'role': return this._guildExecute(interaction);
    case 'pug_role': return this._guildExecute(interaction);
    ...
    case 'Set competition readme': return this._guildExecute(interaction);
    case 'Remove competition readme': return this._guildExecute(interaction);
    }
