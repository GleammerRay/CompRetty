...
export class CompRettyTeams {
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'coin_flip':
      switch (_arg1) {
      case 'captains':
        if (Object.keys(this.#playerTeams).length == 0) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`No players have entered the competition. :empty_nest:`));
        var _teams = Object.values(this.#teams);
        if (_teams.length == 0) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`No players have entered the competition. :empty_nest:`));
        var _fields = [];
        for (let i = 0; i != _teams.length; i++) {
          var _team = _teams[i];
          var _teamName = `:shield: ${_team.name}\n`
          var _players = Object.values(_team.players);
          var _player;
          if (_players.length == 0) _player = 'No players.\n'
          else {
            var _random = ranInt(0, _players.length - 1);
            _player = `<@${_players[_random].id}>`;
          }
          _fields.push({
            name: _teamName,
            value: _player,
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null, moderatorRoleCheck = true) {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null, moderatorRoleCheck = true) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, shuffle, competitionMessageID, logEnabled, startOnShuffle, autoClearThreshold, autoClearDate, moderatorRoleCheck);
  ...
  static fromJSON(json, rest = null) {
    ...
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _shuffle, json.competitionMessageID, json.logEnabled, json.startOnShuffle, json.autoClearThreshold, _autoClearDate, json.moderatorRoleCheck);
    return _competition;
  }
