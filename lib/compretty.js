...
const getHelpMsg = (isAdmin = false) => {
  ...
  return (`{"type":4,"data":{"flags":${1 << 6},"embeds":[{` +
    ...
    '"description":"' +
    `\`/help\` - ${helpDesc}\\n` +
    `\`/competition\`, \`/comp\`, \`/pug\` - ${competitionDesc}\\n` +
    `\`/join\` - ${joinDesc}\\n` +
...
const dmUsers = (rest, userIDs, message) => {
  userIDs.forEach((id) => rest.createDMMessage(id, message));
}
...
export class CompRettyTeams {
  ...
  async onPlayerLeave(interaction) {
    ...
    if (_team == null) {
      ...
      _team.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> left team ${_teamName}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
      this.removePlayer(_playerID);
      this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> left team ${_teamName} (${_team.playerCount})   /   **${this.playerCount} player(s) entered**.`));
      await this._onSave();
  ...
  async _onJoinRandom(interaction) {
    ...
    var _teamName = _team.name;
    _team.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> joined team ${_teamName}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
    this.setPlayerTeam(_playerID, _teamName);
    await this._onSave();
    this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> joined team ${_teamName} (${_team.playerCount})   /   **${this.playerCount} player(s) entered**.`));
  ...
  async updateGuildCommands(interaction) {
    ...
    for (let i = 0; i != _teams.length; i++) {
      ...
      var _teamName = _team.name.toLowerCase().replace(' ', '_');
      if (_teamNames.indexOf(_teamName) != -1) continue;
      var _description;
      if (_team.description == '') _description = 'No description';
      else _description = _team.description;
      _joinTeamCommands.push({
        name: _teamName,
        dm_permission: false,
        description: _description,
        type: 1,
    ...
    var _joinCommand = {
      ...
      description: commands[7].description,
      type: commands[7].type,
      options: [
        {
          name: 'team',
          dm_permission: false,
          description: 'Join a competition team',
          type: 2,
          options: _joinTeamCommands,
        }
      ],
    };
  ...
  async _onJoinTeam(interaction, isAdmin = false, teamName) {
    var _playerID = interaction.member.user.id;
    {
      ...
      if (_lastTeam != null) _lastTeam.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> joined team ${teamName}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
    }
    var _team = this.getTeam(teamName);
    ...
    _team.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> joined team ${teamName}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
    this.setPlayerTeam(_playerID, teamName);
    await this._onSave();
    this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, teamName, CompRettyEventType.Joined, `<@${_playerID}> joined team ${teamName} (${_team.playerCount})   /   **${this.playerCount} player(s) entered**.`));
  }
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'kick':
      ...
      case 'player':
        ...
        var _msg = `<@${interaction.member.user.id}> kicked <@${_playerID}> from team ${_teamName}. :wind_blowing_face:`;
        this.#rest.createDMMessage(_playerID, `{"content":"> ${this.#competitionName}: ${_msg}\\n\\nhttps://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
        this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Left, _msg));
        return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Kicked player <@${_playerID}> from team ${_playerTeam.name} :wind_blowing_face:`));
      }
      return;
    case 'join':
      switch (_arg1) {
      ...
      case 'team':
        ...
        for (let i = 0; i != _teams.length; i++) {
          var _team = _teams[i];
          if (_team == null) continue;
          if (_team.name.toLowerCase().replace(' ', '_') == _arg2) {
            await this._onJoinTeam(interaction, isAdmin, _team.name);
            return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`You have joined team ${_team.name}. :shield:`));
          }
        }
        return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Team with name ${_arg2} no longer exists. :exploding_head:`));
      }
      return;
    ...
    case 'ping':
      ...
      case 'not_ready':
        ...
        _team.dmNotReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> pinged not ready players in team ${_teamName}. :mega:\\n\\nYou can vote to shuffle or ready up at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
        return _response;
      }
    }
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null) {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, shuffle, competitionMessageID, logEnabled, startOnShuffle, autoClearThreshold, autoClearDate);
  ...
  static fromJSON(json, rest = null) {
    ...
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _shuffle, json.competitionMessageID, json.logEnabled, json.startOnShuffle, json.autoClearThreshold, _autoClearDate);
    return _competition;
  }
  ...
  async execute(interaction, isAdmin = false) {
    ...
    if (this.#autoClearDate != null) {
      if (this.#teams.playerCount != 0) {
        ...
        if (_now.getTime() > this.#autoClearDate.getTime()) {
          var _msg = `Competition has been inactive for more than ${this.#autoClearThreshold / 60000} minute(s), all players have been kicked. :cloud_tornado:`;
    ...
    var _playerID = interaction.member.user.id;
    switch (interaction.data.name) {
    case 'join': return this.#teams.execute(interaction, isAdmin);
    case 'leave': return this.#teams.execute(interaction, isAdmin);
    case 'unpug': return this.#teams.execute(interaction, isAdmin);
    case 'force_start':
      if (!isAdmin) return await this.sendPermissionDenied(interaction);
    ...
    if (_customID == null) return await this.sendCompetition(interaction, isAdmin);
    var _command = interaction.data.custom_id.split(' ');
    ...
    if (_arg1 == 'competition') {
      ...
      case 'auto_clear_threshold':
        if (_components != null) {
          ...
          if (typeof _val != 'number') return await this.setupWizard(interaction, true);
