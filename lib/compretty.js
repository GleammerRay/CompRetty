...
const getHelpMsg = (isAdmin = false) => {
  ...
  return (`{"type":4,"data":{"flags":${1 << 6},"embeds":[{` +
    ...
    '"description":"' +
    `\`/help\` - ${helpDesc}\\n` +
    `\`/competition\`, \`/comp\`, \`/join\`, \`/pug\` - ${competitionDesc}\\n` +
...
export class CompRettyEvent {
  ...
  constructor(playerID, type, message) {
...
export class CompRettyTeams {
  ...
  shuffle() {
    ...
    while (_playerIDs.length != 0) {
      ...
      for (let i = 0; i != _currentSortTeams.length; i++) {
        var _team = _currentSortTeams[i];
  ...
  teamsWizard(interaction, isAdmin = false, updateMessage = false, teamName) {
    ...
    return this.#rest.createInteractionResponse(interaction, getInteractionResponse(
      {
        flags: 1 << 6,
        content: `${this.playerCount} players entered this competition.`,
        embeds: [{
          title: teamName,
          description: _description,
          color: embedColor,
            ...
            components: [
              {
                ...
                placeholder: `${_playerPrefix} players (${_playerIDs.length})`,
                options: [
                  {
                    value: 'any',
                    label: `🤗 All players (${_team.playerCount})`,
                    description: 'Show all players',
                  },
                  {
                    value: 'ready',
                    label: `🛡️ Ready players (${Object.keys(_team.readyPlayers).length})`,
                    description: 'Show ready players',
                  },
                  {
                    value: 'not_ready',
                    label: `😴 Not ready players (${Object.keys(_team.notReadyPlayers).length})`,
  ...
  _logEvents(channelID, events) {
    ...
    var _content = '';
    Object.values(events).forEach((event) => _content += `> ${event.message}\\n`);
    this.#rest.createMessage(channelID, `{"content":"${_content}","allowed_mentions":{"parse":[]}}`);
  }

  logEvent(channelID, event) {
    if (!this.#logEnabled) return;
    var _events = {};
    _events[event.playerID] = event;
    ...
    var _events = _logEvents(true);
    var _event = _events[event.playerID];
    if (_event != null) if (event.type == _event.type) return;
    _events[event.playerID] = event;
    burstStack10.set(_stackName, (_channelID = channelID) => {
      return this._logEvents(_channelID, _events);
    });
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'join':
      ...
      if (this.#onSave != null) this.#onSave();
      this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, CompRettyEventType.Joined, `<@${_playerID}> joined team ${_teamName}.`));
      return await this.teamsWizard(interaction, isAdmin, true, _teamName);
    case 'join_random':
      var _availableTeams = [];
    ...
    case 'leave':
      ...
      if (_team != null) this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, CompRettyEventType.Joined, `<@${_playerID}> left team ${_teamName}.`));
      if (_arg3 == 'minimalist') {
    ...
    case 'ping':
      ...
      case 'not_ready':
        ...
        this.#rest.createMessage(interaction.channel_id, `{"content":"> <@${_playerID}> pinged not ready players in team ${_teamName}. :mega:","allowed_mentions":{"parse":[]}}`);
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false) {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, shuffle, competitionMessageID, logEnabled);
  ...
  static fromJSON(json, rest = null) {
    ...
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _shuffle, json.competitionMessageID, json.logEnabled);
    return _competition;
  }
  ...
  async sendCompetition(interaction) {
