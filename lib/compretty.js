...
export class CompRettyTeams {
  ...
  async _onJoinQueue(interaction, isWizard = false) {
    ...
    await this._onSave();
    this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, 'cr_queue', CompRettyEventType.Joined, `<@${_playerID}> joined player queue (${_team.playerCount})   /   **${this.playerCount} player(s) entered**.`));
  }
