...
async function ghostPingUsers(rest, channelID, userIDs) {
...
export class CompRettyTeams {
  ...
  shuffle() {
    ...
    var _availableTeams = Object.keys(this.#teams);
    _playerIDs.forEach((id) => this.removePlayer(id));
    while (_playerIDs.length != 0) {
      var _teamNames = Object.values(_availableTeams);
      var _maxPlayers = Math.floor(_playerIDs.length / _teamNames.length);
      for (let i = _playerIDs.length - 1; i != -1; i--) {
        if (_teamNames.length == 0) break;
        var _id = _playerIDs[i];
        _playerIDs.splice(i, 1);
        var _teamIndex = ranInt(0, _teamNames.length - 1);
        var _teamName = _teamNames[_teamIndex];
        this.setPlayerTeam(_id, _teamName);
        var _team = this.getTeam(_teamName);
        var _playerCount = Object.keys(_team.players).length;
        if (_playerCount == _team.maxPlayers) {
          _teamNames.splice(_teamIndex, 1);
          _availableTeams.splice(_teamIndex, 1);
        }
        else if (_playerCount == _maxPlayers) _teamNames.splice(_teamIndex, 1);
      };
    }
  }
  ...
  teamsWizard(interaction, isAdmin = false, updateMessage = false, teamName) {
    var _options = [];
    ...
    if (_team == null) {
      ...
      if (_playerTeam == null) {
        ...
        _team = this.getTeam(teamName);
        if (_team == null) return this.teamsWizard(interaction, isAdmin, updateMessage, teamName);
      } else {
    ...
    var _playerType;
    if (interaction.data.values != null) _playerType = snakeToPascal(interaction.data.values[0]);
    var _playerPrefix;
    var _playerIDs;
    switch (_playerType) {
    case PlayerType.Ready:
      _playerPrefix = '🛡️ Ready';
      _playerIDs = Object.keys(_team.readyPlayers);
      break;
    case PlayerType.NotReady:
      _playerPrefix = '😴 Not ready'
      _playerIDs = Object.keys(_team.notReadyPlayers);
      break;
    default:
      _playerPrefix = '🤗 All';
      _playerIDs = Object.keys(_team.players);
      break;
    }
    if (_playerIDs.length != 0) {
      if (_description == null) _description = `**${_playerPrefix} players:**\n`;
      else _description += `\n\n**${_playerPrefix} players:**\n`;
    ...
    if (isAdmin) {
      _buttons.push(
        {
          type: 2,
          custom_id: `competition teams ping not_ready "${teamName}"`,
        ...
        components: [
          ...
          },
          {
            ...
            components: [
              {
                type: 3,
                custom_id: `competition teams get_players "${teamName}"`,
                placeholder: `${_playerPrefix} players`,
                options: [
                  {
                    value: 'any',
                    label: '🤗 All players',
                  ...
                  {
                    value: 'ready',
                    label: '🛡️ Ready players',
                    description: 'Show ready players',
                  },
                  {
                    value: 'not_ready',
                    label: '😴 Not ready players',
                    description: 'Show not ready players',
                  },
                ],
              },
            ],
  ...
  async execute(interaction, isAdmin = false) {
    if (Object.keys(this.#teams).length == 0) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`There are no teams set up for this competition. :empty_nest:`));
    ...
    case 'kick':
      ...
      case 'all':
        this.clearPlayers();
        if (this.#onSave != null) this.#onSave();
        return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Kicked all players from all teams :cloud_tornado:`));
      case 'player':
        var _playerID = _option.options[0].value;
        var _playerTeam = this.getPlayerTeam(_playerID);
        if (_playerTeam == null) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Player is not a member of any team :face_in_clouds:`));
        this.removePlayer(_playerID);
        if (this.#onSave != null) this.#onSave();
        return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Kicked player <@${_playerID}> from team ${_playerTeam.name} :wind_blowing_face:`));
      }
      return;
    }
    var _playerID = interaction.member.user.id;
    ...
    case 'join_random':
      ...
      var _teamName = _availableTeams[_teamIndex].name;
      this.setPlayerTeam(_playerID, _teamName);
      if (this.#onSave != null) this.#onSave();
    ...
    case 'leave':
      ...
      if (_arg3 == 'minimalist') {
        ...
        return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`You have left team ${_teamName}. :levitate:`));
      }
      return await this.teamsWizard(interaction, isAdmin, true, _teamName);
    case 'ping':
      if (!isAdmin) return this.teamsWizard(interaction, isAdmin, true);
      switch (_arg3) {
      case 'not_ready':
        var _team = this.getTeam(_teamName);
        if (_team == null) return await this.teamsWizard(interaction, isAdmin, true);
        var _response = await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Pinging not ready players in team ${_teamName}. :mega:`));
        _team.ghostPingNotReadyPlayers(interaction.channel_id);
        return _response;
      }
    }
    if (interaction.data.values == null) return await this.teamsWizard(interaction, isAdmin);
...
export class CompRettyShuffle {
  ...
  static fromDB(path, rest = null) {
    ...
    if (Fs.existsSync(_jsonPath)) {
      ...
      _shuffle = CompRettyShuffle.withREST(rest, new CompRettyTeams());
      _shuffle.#dbPath = path;
      _shuffle.#jsonPath = _jsonPath;
      _shuffle.saveSync();
    }
  ...
  static fromJSON(json, rest = null) {
    return CompRettyShuffle.withREST(rest, CompRettyTeams.fromJSON(json.teams), CompRettyVotes.fromJSON(json.votes));
  }
  ...
  rmSync() {
    if (this.#dbPath != null) Fs.rmSync(this.#jsonPath);
  }
  ...
  shuffleWizard(interaction, isAdmin = false, updateMessage = false, teamName = null) {
    ...
    this.#rest.createInteractionResponse(interaction, getInteractionResponse({
      flags: 1 << 6,
      embeds: [_teamEmbed],
  ...
  execute(interaction, isAdmin = false) {
    var _playerID = interaction.member.user.id;
    if (this.#votes.getVote(_playerID) == null) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse('You are not participating in this vote. :no_entry_sign:'));
    var _command = interaction.data.custom_id.split(' ');
    var _arg1 = _command[1];
    var _arg2 = _command[2];
    var _arg3 = _command[3];
    switch (_arg1) {
    case undefined: return this.shuffleWizard(interaction, isAdmin);
    case 'refresh': return this.shuffleWizard(interaction, isAdmin, true);
    case 'vote':
      switch (_arg2) {
      case 'for':
        if (isAdmin) {
          if (_arg3 == 'force') {
            ...
            var _response = this.#rest.createInteractionResponse(interaction, getInteractionResponse({content: 'The shuffle has finished. :game_die:', embeds: [], components: []}, 7));
            this.#rest.createMessage(this.#channelID, JSON.stringify({ content: `<@${_playerID}> shuffled teams. :game_die:` }));
            return _response;
          }
        }
        ...
        if (this.#votes.forCount == this.#votes.requiredVotesCount) {
          ...
          if (this.#onFinished != null) this.#onFinished(this.#teams);
          var _response = this.#rest.createInteractionResponse(interaction, getInteractionResponse({content: 'The shuffle has finished. :game_die:', embeds: [], components: []}, 7));
          this.#rest.createMessage(this.#channelID, JSON.stringify({ content: 'The majority has voted to shuffle. :game_die:' }));
          return _response;
        }
        this.saveSync();
        return this.shuffleWizard(interaction, isAdmin, true);
      case 'against':
        if (isAdmin) {
          if (_arg3 == 'force') {
            this.rmSync();
            if (this.#onFinished != null) this.#onFinished();
            var _response = this.#rest.createInteractionResponse(interaction, getInteractionResponse({content: 'The shuffle has finished. :game_die:', embeds: [], components: []}, 7));
            this.#rest.createMessage(this.#channelID, JSON.stringify({ content: `<@${_playerID}> cancelled the shuffle. :sleeping:` }));
            return _response;
          }
        }
        ...
        if (this.#votes.againstCount == this.#votes.requiredVotesCount) {
          this.rmSync();
          if (this.#onFinished != null) this.#onFinished();
          var _response = this.#rest.createInteractionResponse(interaction, getEphemeralResponse('The shuffle has finished. :game_die:'));
          this.#rest.createMessage(this.#channelID, JSON.stringify({ content: 'The majority has voted to cancel the shuffle. :sleeping:' }));
          return _response;
        }
        this.saveSync();
        return this.shuffleWizard(interaction, isAdmin, true);
      }
      return this.shuffleWizard(interaction, isAdmin);
    case 'get':
      if (_arg2 == 'team') {
        var _teamName = interaction.data.values[0];
        return this.shuffleWizard(interaction, isAdmin, true, _teamName);
      }
      return this.shuffleWizard(interaction, isAdmin);
    }
    return this.shuffleWizard(interaction, isAdmin);
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionInteraction = null) {
    ...
    this.#competitionInteraction = competitionInteraction;
  }

  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionInteraction = null) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, shuffle, competitionInteraction);
  ...
  static fromJSON(json, rest = null) {
    ...
    if (json.shuffle != null) {
      _shuffle = CompRettyShuffle.fromJSON(json.shuffle);
      _shuffle.channelID = this.#channelID;
      _shuffle.onFinished = (teams) => {
        if (_competition == null) return;
        if (teams == null) return;
        _competition.#teams = teams;
        _competition.saveSync();
      }
    } else _shuffle = null;
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _shuffle, json.competitionInteraction);
    return _competition;
  }
  ...
  toJSON() {
    return {
      ...
      competitionInteraction: this.#competitionInteraction,
    };
  }
  ...
  getOrCreateShuffle() {
    if (this.#shuffle == null) {
      if (this.#dbPath != null) {
        ...
        var _newTeams = CompRettyTeams.fromJSON(this.#teams.toJSON());
        _newTeams.shuffle();
  ...
  cancelShuffle() {
    if (this.#dbPath == null) this.#shuffle = null;
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'kick': return await this.#teams.execute(interaction, isAdmin);
    case 'shuffle':
      ...
      if (!this.shuffleExists) this.getOrCreateShuffle();
      return await this.#rest.createInteractionResponse(interaction, getInteractionResponse({
        embeds:[{
    ...
    if (_customID == null) return await this.sendCompetition(interaction);
    var _command = interaction.data.custom_id.split(' ');
    ...
    case 'shuffle':
      ...
      return await this.getOrCreateShuffle().execute(interaction, isAdmin);
    }
    var _arg1 = _command[1];
    if (_arg1 == 'teams') return await this.#teams.execute(interaction, isAdmin);
  }
  ...
  _getCompetitionMessage(_hasControls = true, _hasPlayerCount = true) {
    ...
    var _description = '';
    if (_hasPlayerCount) _description += `**${this.#teams.playerCount} players entered.**\n\n`;
    if (this.#description != '') _description += `${this.#description}\n\n`;
  ...
  updateCompetitionMessage() {
    if (this.#competitionInteraction == null) return;
    this.#rest.editOriginalInteractionResponse(this.#competitionInteraction, JSON.stringify(this._getCompetitionMessage(true, true)));
  }

  sendCompetition(interaction) {
    if (this.#competitionInteraction != null) this.#rest.editOriginalInteractionResponse(this.#competitionInteraction, JSON.stringify(this._getCompetitionMessage(true, false)));
    this.#competitionInteraction = interaction;
    var _result = this.#rest.createInteractionResponse(interaction, getInteractionResponse(this._getCompetitionMessage(true, true)));
    this.saveSync();
    return _result;
  }
  ...
  async _onAllTeamsReady(interaction) {
    ...
    await this.#teams.ghostPingPlayers(interaction.channel_id);
    this.#teams.clearPlayers();
    this.#enabled = true;
    this.saveSync();
  }
