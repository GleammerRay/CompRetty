...
export class CompRettyTeams {
  ...
  async onPlayerLeave(interaction) {
    ...
    if (_team == null) {
      ...
      this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> left team ${_teamName}.`));
      await this._onSave();
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'join':
      {
        ...
        if (_lastTeam != null) _lastTeam.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> joined team ${_teamName}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
      }
      ...
      await this._onSave();
      this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> joined team ${_teamName} (${_team.playerCount})   /   ${this.playerCount} total player(s).`));
      return await this.teamsWizard(interaction, isAdmin, true, _teamName);
    case 'join_random':
      var _availableTeams = [];
    ...
    case 'leave':
      ...
      if (_team == null) _teamName = '';
        ...
        _team.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> left team ${_teamName}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
        this.removePlayer(_playerID);
        this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> left team ${_teamName} (${_team.playerCount})   /   ${this.playerCount} total player(s).`));
        await this._onSave();
      }
      if (_arg3 == 'minimalist') {
