...
export class CompRettyTeams {
  ...
  teamsWizard(interaction, isAdmin = false, updateMessage = false, teamName) {
    ...
    if (_team == null) {
      _team = this.getPlayerTeam(_playerID);
      if (_team == null) {
        teamName = _options[0].label;
        _team = this.getTeam(teamName);
        if (_team == null) return this.teamsWizard(interaction, isAdmin, updateMessage);
      } else {
        teamName = _team.name;
        _team = _playerTeam;
      }
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'leave':
      ...
      return await this.teamsWizard(interaction, isAdmin, true, _teamName);
    case 'ping':
      if (!isAdmin) return this.teamsWizard(interaction, isAdmin, true, _teamName);
      switch (_arg3) {
      case 'not_ready':
        var _team = this.getTeam(_teamName);
        if (_team == null) return await this.teamsWizard(interaction, isAdmin, true, _teamName);
