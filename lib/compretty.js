...
export class CompRettyTeams {
  ...
  async onPlayerLeave(interaction) {
    ...
    if (_team == null) {
      ...
      _team.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> left ${_teamName == 'Queue' ? 'player queue' : `team ${_teamName}`}.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
      this.removePlayer(_playerID);
      this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Joined, `<@${_playerID}> left ${_teamName == 'Queue' ? 'player queue' : `team ${_teamName}`} (${_team.playerCount})   /   **${this.playerCount} player(s) entered**.`));
      await this._onSave();
      return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`You have left ${_teamName == 'Queue' ? 'player queue' : `team ${_teamName}`}. :levitate:`));
    }
  }
  ...
  async _onJoinQueue(interaction, isAdmin = false) {
    var _playerID = interaction.member.user.id;
    var _team = this.#queue;
    if (_team == null) return await this.teamsWizard(interaction, isAdmin, true);
    if (_team.getPlayer(_playerID) != null) return await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`You are already in player queue. :hourglass:`));
    {
      var _lastTeam = this.getPlayerTeam(_playerID);
      if (_lastTeam != null) {
        _lastTeam.dmReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> joined player queue.\\n\\nYou are no longer marked ready, make sure that the team arrangement is still good at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
    ...
    await this._onSave();
    this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, 'Queue', CompRettyEventType.Joined, `<@${_playerID}> joined player queue (${_team.playerCount})   /   **${this.playerCount} player(s) entered**.`));
  }
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'join':
      ...
      case 'queue':
        this._onJoinQueue(interaction, isAdmin);
    ...
    case 'join': {
      if (_teamName == 'cr_queue') {
        if (await this._onJoinQueue(interaction, isAdmin) != null) return;
    ...
    case 'ping':
      ...
      switch (_arg3) {
      case 'not_ready':
        var _response = await this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Pinging not ready players in ${_teamName == 'Queue' ? 'player queue' : `team ${_teamName}`}. :mega:`));
        this.#rest.createMessage(interaction.channel_id, `{"content":"> <@${_playerID}> pinged not ready players in ${_teamName == 'Queue' ? 'player queue' : `team ${_teamName}`}. :mega:","allowed_mentions":{"parse":[]}}`);
        _team.dmNotReadyPlayers(`{"content":"> ${this.#competitionName}: <@${_playerID}> pinged not ready players in ${_teamName == 'Queue' ? 'player queue' : `team ${_teamName}`}. :mega:\\n\\nYou can vote to shuffle or ready up at https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}"}`);
        return _response;
      }
    }
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, queue = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null, moderatorRoleCheck = true) {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, queue = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false, autoClearThreshold = null, autoClearDate = null, moderatorRoleCheck = true) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, queue, shuffle, competitionMessageID, logEnabled, startOnShuffle, autoClearThreshold, autoClearDate, moderatorRoleCheck);
  ...
  static fromJSON(json, rest = null) {
    ...
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _queue, _shuffle, json.competitionMessageID, json.logEnabled, json.startOnShuffle, json.autoClearThreshold, _autoClearDate, json.moderatorRoleCheck);
    return _competition;
  }
      ...
      case 'queue':
        ...
        case 'enable':
          ...
          case 'true':
            ...
            var _queue = new CompRettyTeam('Queue', 'Player queue');
            this.#queue = _queue;
  ...
  _getCompetitionMessage(_hasControls = true, _hasPlayerCount = true) {
    ...
    if (_hasControls) {
      ...
      if (this.#queue != null) {
        _components[0].components.splice(2, 0, {
          type: 2,
