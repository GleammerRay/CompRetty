  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'kick':
      ...
      case 'all':
        this.clearPlayers();
        if (this.#onSave != null) this.#onSave();
        this.logEvent(interaction.channel_id, new CompRettyEvent(null, null, CompRettyEventType.Left, `<@${interaction.member.user.id}> kicked all players from all teams. :cloud_tornado:`));
      ...
      case 'player':
        ...
        if (this.#onSave != null) this.#onSave();
        this.logEvent(interaction.channel_id, new CompRettyEvent(_playerID, _teamName, CompRettyEventType.Left, `<@${interaction.member.user.id}> kicked <@${_playerID}> from team ${_teamName}. :wind_blowing_face:`));
...
export class CompRettyShuffle {
  ...
  static fromJSON(json, rest = null) {
    var _teams = {};
    Object.values(json.teams).forEach((team) => _teams[team.name] = CompRettyTeam.fromJSON(team, rest));
    return CompRettyShuffle.withREST(rest, new CompRettyTeams(_teams), CompRettyVotes.fromJSON(json.votes));
  }
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false) {
    ...
    if (shuffle != null) shuffle.onFinished = (teams) => this._onShuffle(teams);
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, competitionRole = null, teams = null, shuffle = null, competitionMessageID = null, logEnabled = false, startOnShuffle = false) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, competitionRole, teams, shuffle, competitionMessageID, logEnabled, startOnShuffle);
  ...
  static fromJSON(json, rest = null) {
    ...
    var _shuffle;
    var _competition;
    if (json.shuffle != null) _shuffle = CompRettyShuffle.fromJSON(json.shuffle);
    else _shuffle = null;
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, json.competitionRole, _teams, _shuffle, json.competitionMessageID, json.logEnabled, json.startOnShuffle);
    return _competition;
  }
  ...
  async _onAllTeamsReady(interaction) {
    ...
    await this.#teams.ghostPingPlayers(_channelID);
    this.clearPlayers();
    this.#enabled = true;
    this.saveSync();
  }
