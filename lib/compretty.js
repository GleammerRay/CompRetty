...
export class CompRettyCompetition {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, teams = null, shuffle = null) {
    ...
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, teams, shuffle = null);
  ...
  static fromJSON(json, rest) {
    ...
    return CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, _teams, json.shuffle);
  }
  ...
  execute(interaction, isAdmin = false) {
    ...
    if (interaction.data.name == 'kick') return this.#teams.execute(interaction, isAdmin);
    var _customID = interaction.data.custom_id;
    if (_customID == null) return this.sendCompetition(interaction);
    var _command = interaction.data.custom_id.split(' ');
    var _arg0 = _command[0];
    if (_arg0 == 'setup') return this.setup(interaction, isAdmin);
...
export class CompRetty {
  ...
  constructor(botToken, ownerID) {
    ...
    this.#userCommands = {
      ...
      kick: (interaction) => this._guildExecute(interaction),
    };
  }
