...
export class CompRettyTeams {
  ...
  get playerCount() {
    var _playerCount = 0;
    Object.values(this.#teams).forEach((team) => _playerCount += Object.keys(team.players).length);
    return _playerCount
  }
...
export class CompRettyCompetition {
  ...
  constructor(name, description = '', imageURL = '', shuffleEnabled = true, teams = null, shuffle = null) {
  ...
  static withREST(rest, name, description = '', imageURL = '', shuffleEnabled = true, teams = null, shuffle = null) {
    if (teams == null) teams = CompRettyTeams.withREST(rest);
    var _competition = new CompRettyCompetition(name, description, imageURL, shuffleEnabled, teams, shuffle);
  ...
  static fromJSON(json, rest = null) {
    ...
    _competition = CompRettyCompetition.withREST(rest, json.name, json.description, json.imageURL, json.shuffleEnabled, _teams, _shuffle);
    return _competition;
  }
  ...
  execute(interaction, isAdmin = false) {
    ...
    var _playerID = interaction.member.user.id;
    switch (interaction.data.name) {
    case 'kick': return this.#teams.execute(interaction, isAdmin);
    case 'shuffle':
      if (!this.#shuffleEnabled) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`Shuffle is disabled for this competition. :no_entry_sign:`));
      if (this.#teams.getPlayerTeam(_playerID) == null) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse(`You must be a member of a team to start a shuffle. :spy:`));
    ...
    case 'shuffle':
      if (!this.shuffleExists) return this.#rest.createInteractionResponse(interaction, getEphemeralResponse('There is no shuffle in progress. You can start a new shuffle with `/shuffle`. :sleeping:'));
      return this.getOrCreateShuffle().execute(interaction, isAdmin);
    }
    var _arg1 = _command[1];
    if (_arg1 == 'teams') return this.#teams.execute(interaction, isAdmin);
  }
