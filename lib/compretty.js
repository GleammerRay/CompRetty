...
const getFirstRole = str => {
  ...
  var _reading = false;
  var _role = '';
  console.log(/^\d+$/.test('1test1'));
  console.log(/^\d+$/.test('111'));
...
export class CompRettyTeams {
  ...
  async _onKickPlayer(interaction, playerID) {
    ...
    var _msg = `<@${interaction.member.user.id}> kicked <@${playerID}> from team ${_teamName}. :wind_blowing_face:`;
...
export class CompRettyCompetition {
  ...
  async execute(interaction, isAdmin = false) {
    ...
    case 'shuffle':
      ...
      return _response;
    case 'Kick player': return this.#teams.execute(interaction, isAdmin);
    case 'Kick mentioned player': return this.#teams.execute(interaction, isAdmin);
    }
